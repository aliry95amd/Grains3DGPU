...
/** @brief The class LinkedCell_SortBased.
    ...
    on sorting the particle hashes. This is designed to work on the device
   (GPU). This has optimal space complexity, while the time complexity is O(n)
   for computing the particle hashes and O(nk) for sorting the particle IDs
   using a parallel radix sort algorithm, where n is the number of particles and
...
public:
    ...
    {
        GAbort("LinkedCell_SortBased::getNumParticlesPrefixSums is not supported");
        return nullptr;
    }
    //@}
    ...
    bool updateLinkedCells()
    {
        ...
        thrust::sort_by_key(thrust::device_ptr<uint>(m_cellID.getData()),
        ...
        computeCellStart_Kernel<<<numBlocks, numThreads, sMemSize>>>(m_cellID.getData(),
                                                                     m_numParticles,
