# ===========================================================================
# ARC HPC Module Environment
# Load all required modules up front so this file is fully self-contained.
# Just "source grainsGPU_arc.env.sh" from a clean shell to set everything up.
# ===========================================================================
# Initialise Lmod if not already available (needed in non-interactive shells
# like SLURM job scripts where .bashrc may not run).
if ! command -v module >/dev/null 2>&1; then
    source /etc/profile.d/modules.sh 2>/dev/null || true
    source /etc/profile.d/zz-60-lmod.sh 2>/dev/null || true
fi
# Load nvhpc/23.9 — provides the CUDA 12.2 toolkit (nvcc, CUB, curand, etc.)
module load nvhpc/23.9 2>/dev/null || true

# Definition
# Load nvhpc/23.9 to get the CUDA 12.2 toolkit (nvcc, CUB, curand, etc.).
# We use nvcc as the GPU compiler and GCC 9.4.0 as the host/CPP compiler.
# GCC 9.4.0 is used as the host compiler for nvcc and for building Xerces.
...
# GPU — nvcc from the CUDA 12.2 toolkit bundled with nvhpc/23.9
...
# Release mode flags (nvcc with GCC 9.4.0 as host compiler)
